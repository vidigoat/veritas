...
const V_A = ["Apex", "Zenith", "Crest", "Summit", "Pioneer", "Vertex", "Orion", "Stellar",
  "Cascade", "Meridian", "Horizon", "Quantum", "Falcon", "Everest", "Lotus", "Sapphire",
  "Coromandel", "Nova", "Deccan", "Prism", "Konark", "Godavari", "Sahyadri", "Trident",
  "Kaveri", "Aravalli", "Nilgiri", "Bharat", "Shakti", "Vega", "Ashwin", "Ganga"];
...
function makeEmployees(rng: Rng): Employee[] {
  const dir: Omit<Employee, "home" | "account" | "email">[] = [
    { id: "E-001", name: "Rohan Sharma",     role: "Operations Head",              dept: "Operations",  joined: "2016-06-13", salary: 165000, manager: "E-004" },
    { id: "E-002", name: "Priya Mehta",       role: "Sales Manager",                dept: "Sales",       joined: "2017-02-20", salary: 118000, manager: "E-004" },
    { id: "E-003", name: "Aditya Deshpande",  role: "Finance Controller",           dept: "Finance",     joined: "2016-09-01", salary: 152000, manager: "E-004" },
    { id: "E-004", name: "Sneha Iyer",        role: "Managing Director",            dept: "Executive",   joined: "2016-04-01", salary: 240000, manager: "E-004" },
    { id: "E-005", name: "Karan Malhotra",    role: "Warehouse Supervisor",         dept: "Operations",  joined: "2018-11-05", salary: 58000,  manager: "E-001" },
    { id: "E-006", name: "Ananya Nair",       role: "HR Manager",                   dept: "HR",          joined: "2018-03-19", salary: 96000,  manager: "E-004" },
    { id: "E-007", name: "Vikram Kulkarni",   role: "Procurement & Finance Manager", dept: "Procurement", joined: "2019-07-22", salary: 98000,  manager: "E-003", fraudster: true },
    { id: "E-008", name: "Rajesh Gupta",      role: "Accounts Payable Clerk",       dept: "Finance",     joined: "2020-01-15", salary: 46000,  manager: "E-003" },
    { id: "E-009", name: "Meera Reddy",       role: "Logistics Coordinator",        dept: "Operations",  joined: "2019-10-08", salary: 52000,  manager: "E-001" },
    { id: "E-010", name: "Arjun Joshi",       role: "Quality Lead",                 dept: "Operations",  joined: "2020-08-11", salary: 74000,  manager: "E-001" },
    { id: "E-011", name: "Pooja Desai",       role: "Admin Executive",              dept: "Admin",       joined: "2021-05-24", salary: 42000,  manager: "E-006" },
    { id: "E-012", name: "Sanjay Rao",        role: "IT Manager",                   dept: "IT",          joined: "2019-12-02", salary: 108000, manager: "E-004" },
    { id: "E-013", name: "Nisha Chopra",      role: "Sales Executive",              dept: "Sales",       joined: "2022-01-17", salary: 44000,  manager: "E-002" },
    { id: "E-014", name: "Amit Bose",         role: "Accounts Executive",           dept: "Finance",     joined: "2021-09-06", salary: 48000,  manager: "E-003" },
    { id: "E-015", name: "Deepak Verma",      role: "Data Entry Operator",          dept: "Procurement", joined: "2026-01-05", salary: 72000,  manager: "E-007", ghost: true },
    { id: "E-016", name: "Kavya Menon",       role: "Procurement Executive",        dept: "Procurement", joined: "2022-06-28", salary: 47000,  manager: "E-007" },
    { id: "E-017", name: "Rahul Singh",       role: "Marketing Manager",            dept: "Marketing",   joined: "2020-04-14", salary: 102000, manager: "E-004" },
    { id: "E-018", name: "Shreya Patil",      role: "Customer Support Lead",        dept: "Sales",       joined: "2021-11-30", salary: 56000,  manager: "E-002" },
  ];
  ...
  return dir.map((e) => {
    ...
    const email = e.ghost ? null
      : `${e.name.toLowerCase().replace(/[^a-z]+/g, ".")}@northwind-trading.co`;
...
function makeVendors(rng: Rng, e007: Employee): Vendor[] {
  ...
  for (let i = 1; i <= 110; i++) {
    ...
    let name = "";
    do { name = `${rng.pick(V_A)} ${rng.pick(V_B)}`; } while (usedName.has(name));
    usedName.add(name);
...
function buildBooks(rng: Rng, vendors: Vendor[], employees: Employee[]) {
  ...
  /* --- H2 herring: Rs 250,000 CAPEX, fully authorised (PO-77001 + board) --- */
  ...
  const shellTotals = [24800, 24950, 24500, 24650, 24900, 24750, 24850, 24200, 23400, 22800, 21600, 19900, 24400, 22300]; // sum = 332,000
  const shellMemos = [
  ...
  for (let i = 0; i < 14; i++) {
    ...
    invoices.push({
      ...
      vendorRef: `APEX/25-26/${String(apexRef++).padStart(3, "0")}`,
...
function rPurchaseOrder(po: PurchaseOrder, v: Vendor): string {
  ...
  return [
    ...
    po.note ? `Note: ${po.note}` : `Delivery: ex-works, freight to be arranged by Northwind.`,
  ].join("\n");
}
...
function rBoardMinutes(doc: { file: string; date: string; title: string; body: string[] }): string {
  return [
    ...
    `        Sneha Iyer, Chair          Aditya Deshpande, Finance Controller`,
  ].join("\n");
}
...
function main() {
  ...
  docs.push({ name: "CREDIT-NOTE-CR-1042.txt", content: rCreditNote("CR-1042", "2025-07-12", crRevVendor, 54200, "Duplicate payment identified during weekly bank reconciliation; second payment reversed.", "Duplicate of July invoice payment", "E-003") });
  ...
  console.log(`\n${rule("=")}`);
  console.log(`VERITAS demo corpus - ${CO.name}, FY 2025-26 (seed 4471)`);
  console.log(rule("="));
  ...
  console.log(`  1. SHELL      V-031 Apex Supplies - ${invoices.filter((i) => i.tag === "shell").length} invoices - ${rupee(shellTotal)}`);
  console.log(`     exact match: "${FRAUDSTER_HOME}" in VENDOR-V031-registration.txt & HR-E007-record.txt`);
  console.log(`  2. GHOST      E-015 Deepak Verma - salary ${rupee(e015.salary)}/mo`);
  ...
  console.log(`  4. REVERSED dup ${rupee(54200)} to ${byId.get("V-006")!.name} -> CREDIT-NOTE-CR-1042.txt`);
