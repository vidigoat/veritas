...
const MODELS = [
  'Qwen/Qwen3.5-397B-A17B',        // Vultr-native senior candidate
  'Qwen/Qwen3.6-27B',              // Vultr-native junior candidate
  'moonshotai/Kimi-K2.6',          // current senior (Vultr-served baseline)
  'nvidia/Nemotron-Cascade-2-30B-A3B', // verifier/junior baseline (keep for NVIDIA)
];
...
async function call(model, messages, useTools = true) {
  ...
  const r = await fetch(URL_, { method: 'POST', headers: { Authorization: `Bearer ${KEY}`, 'Content-Type': 'application/json' },
    body: JSON.stringify({ model, messages: [{ role: 'system', content: SYS }, ...messages], ...(useTools ? { tools: TOOLS } : {}), temperature: 0.1, max_tokens: 2000 }) });
