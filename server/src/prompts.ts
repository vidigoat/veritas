...
export const PHASE_HINTS: Record<string, string> = {
  ...
  investigate: "Work each open hypothesis to a verdict using the examiner's method. For each suspect: FIRST try to EXONERATE it (call exonerate, search for the innocent explanation — real service? shared address? POs elsewhere? reversal? authorization?). If you find an innocent explanation, CLEAR it (update_hypothesis status=cleared, cite the doc). If you cannot exonerate it AND cannot rule out all five innocent causes, keep it UNPROVEN. Only when the fraud theory holds and every innocent explanation is ruled out, mark it confirmed. Use search_documents to RETRIEVE the pages that settle each question — it is powered by VultronRetriever, which reads the full page (layout, addresses, tax IDs, PO references) and surfaces the exact document even when your query words never appear on it. For a suspected shell, retrieve the vendor registration and the approving employee's record and compare the fields. Pull vendor_profile, get_document, trace_payments as evidence demands. One lead at a time; after each result state in one line what changed and what you need next.",
